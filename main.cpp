...
//
//  使用方法：
//  1. 使用Xcode创建一个C++ Project，将原本的main.cpp文件删除后再将此main.cpp文件拖入
//  2. 在Xcode右边菜单栏打开本文件的Target Membership
//  3. 在Edit Scheme-Options中调整本程序的Working Directory，将需要处理的CSV文件移动到与本程序相同的Working Directory
//  4. 务必将CSV文件名改为英文或数字，并确保在之后的输入中名字正确，并在初始数据导出时确保CSV中的数据都是文本格式
//  5. 点击Xcode上方菜单栏的“播放”按钮启动程序
//  6. 按程序提示进行一步步的操作
//  7. 程序结束后前往Working Directory查看result.txt文件
...
int main(int argc, const char * argv[]) {
    // Intro
    int num_files = 0;
    cout << "欢迎使用CSV数据整理工具，本程序可以读取多个百万量级的CSV文件，将每个CSV文件首列去重，然后将去重后的数据整合到一个文本txt文件里\n";
    ...
    cout << "请确保本程序所在的文件夹没有已存在名叫result.txt的文件\n";
    ...
    cout << "\n";
    cout << "*******************************************************\n";
    cout << "***整理结束，请前往程序所在文件夹查看result.txt文件，谢谢使用***\n";
    cout << "*******************************************************\n";
    cout << "\n";
